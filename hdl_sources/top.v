...
module top (
    ...
    // -----------------------------------------------------------------------
    // BRAM to VGA Direct Mapping (Pixel Doubling 320x240 -> 640x480)
    // -----------------------------------------------------------------------
    wire [8:0] buf_col = hcount[9:1]; // hcount / 2
    wire [7:0] buf_row = vcount[9:1]; // vcount / 2
    
    // Address = buf_row * 320 + buf_col
    assign rd_addr = (active) ? (({9'd0, buf_row} << 8) + ({9'd0, buf_row} << 6) + {8'd0, buf_col}) : 17'd0;

    // Pipeline active signal to match BRAM latency (2 cycles)
    ...
    always @(posedge pixel_clk) begin
        // Use active_d2 so we only capture valid BRAM output
        if (pix_rst || !active_d2 || !frame_valid) begin
            r_out <= 4'd0;
            g_out <= 4'd0;
            b_out <= 4'd0;
        end else begin
            r_out <= filtered_data[11:8];
            g_out <= filtered_data[7:4];
            b_out <= filtered_data[3:0];
        end
    end
    ...
    // -----------------------------------------------------------------------
    // VGA Sync Delay Compensation (3 cycles)
    // Align hsync/vsync with the 3-cycle pipeline:
    // Cycle 1: BRAM internal read
    // Cycle 2: BRAM output register
    // Cycle 3: r_out register
    // -----------------------------------------------------------------------
    reg [2:0] hsync_delay, vsync_delay;
    always @(posedge pixel_clk) begin
        if (pix_rst) begin
            hsync_delay <= 3'b111;
            vsync_delay <= 3'b111;
        end else begin
            hsync_delay <= {hsync_delay[1:0], hsync_raw};
            vsync_delay <= {vsync_delay[1:0], vsync_raw};
        end
    end
